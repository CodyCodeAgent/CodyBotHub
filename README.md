...
- 原子“工具”以固定可执行文件和 argv 模板注册，支持 Bits RPC 与本地命令；执行不经过 Shell，并限制超时和输出大小
...
工具是最小执行单元，建议一个工具只对应一个明确接口。`command` 填可执行文件，参数模板每行对应一个 argv，不会经过 Shell 展开。模板中的 `{{field}}` 从运行参数取值，`{{json}}` 会写入完整参数 JSON。例如 Bits RPC 可以配置为：
