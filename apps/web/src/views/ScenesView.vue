...
const pageSize = 20
const items = ref<Scene[]>([]), bots = ref<Bot[]>([]), workspaces = ref<Workspace[]>([]), packages = ref<SkillPackage[]>([]), messageTypeOptions = ref<MessageTypeOption[]>([]), total = ref(0), offset = ref(0), loading = ref(false), open = ref(false), saving = ref(false), error = ref('')
...
const inheritedModel = computed(() => selectedBot.value?.model || platform.value.defaultModel || catalog.value.defaultModel)
...
const load = async () => { loading.value = true; try { const [page, botItems, workspaceItems, packageItems, typeItems, models, settings] = await Promise.all([api.scenesPage(pageSize, offset.value), api.bots(), api.workspaces(), api.skillPackages(), api.feishuMessageTypes(), api.models(), api.settings()]); items.value = page.items; total.value = page.total; bots.value = botItems; workspaces.value = workspaceItems; packages.value = packageItems; messageTypeOptions.value = typeItems; catalog.value = models; platform.value = settings } finally { loading.value = false } }
...
const edit = (item?: Scene) => {
  Object.assign(form, item ? { ...item, chatIdsText: item.matcher.chatIds.join('\n'), messageTypes: [...item.matcher.messageTypes], textIncludesText: item.matcher.textIncludes.join('\n'), cardTitleIncludesText: item.matcher.cardTitleIncludes.join('\n'), skillPackageIds: [...item.skillPackageIds], retrievalBoostsText: item.retrieval.skillBoosts.map(value => `${value.keyword}=${value.weight}`).join('\n'), skillCandidateLimit: item.retrieval.skillCandidateLimit, knowledgeCandidateLimit: item.retrieval.knowledgeCandidateLimit, minimumScore: item.retrieval.minimumScore } : { id: '', botId: bots.value[0]?.id ?? '', workspaceId: bots.value[0]?.defaultWorkspaceId ?? '', name: '', prompt: '', priority: 100, enabled: true, model: '', reasoningEffort: '', chatIdsText: '', messageTypes: [], textIncludesText: '', cardTitleIncludesText: '', skillPackageIds: [], retrievalBoostsText: '', skillCandidateLimit: 12, knowledgeCandidateLimit: 12, minimumScore: 1 })
  error.value = ''; open.value = true; void loadChats(form.botId)
}
const lines = (value: string) => value.split('\n').map(v => v.trim()).filter(Boolean)
const parseBoosts = () => lines(form.retrievalBoostsText).map(line => { const match = line.match(/^(.+?)(?:\s*[=:：]\s*(\d+(?:\.\d+)?))?$/u); const keyword = match?.[1]?.trim() ?? ''; const weight = Number(match?.[2] ?? 10); if (!keyword || !Number.isFinite(weight) || weight <= 0 || weight > 100) throw new Error(`检索增强格式错误：${line}`); return { keyword, weight } })
const save = async () => { saving.value = true; error.value = ''; try { const chatIds = lines(form.chatIdsText); if (chatIds.some(value => value.startsWith('@'))) throw new Error('请从候选中选择群，或填写完整 Chat ID'); await api.saveScene({ ...form, retrieval: { skillBoosts: parseBoosts(), skillCandidateLimit: form.skillCandidateLimit, knowledgeCandidateLimit: form.knowledgeCandidateLimit, minimumScore: form.minimumScore }, matcher: { chatIds, messageTypes: form.messageTypes, textIncludes: lines(form.textIncludesText), cardTitleIncludes: lines(form.cardTitleIncludesText) } }); open.value = false; await load() } catch (e) { error.value = e instanceof Error ? e.message : '保存失败' } finally { saving.value = false } }
...
<template>
  <div class="page">
    ...
    <section class="panel"><div class="panel-header"><h2>全部场景</h2><span class="badge gray">{{ total }} 个</span></div><div v-if="!loading && !items.length" class="empty"><Workflow :size="32" /><strong>还没有场景</strong><span>可以按群、消息类型、文本或卡片标题进行匹配。</span></div><div v-else class="table-wrap"><table><thead><tr><th>场景</th><th>Bot / 工作区</th><th>匹配条件</th><th>模型 / 优先级</th><th aria-label="操作" /></tr></thead><tbody><tr v-for="item in items" :key="item.id"><td><div class="entity-title"><CircleDot :size="12" :color="item.enabled ? 'var(--green)' : 'var(--muted)'" style="vertical-align:-1px;margin-right:6px" />{{ item.name }}</div><div class="entity-subtitle">{{ item.enabled ? '已启用' : '已停用' }} · 每轮动态路由</div></td><td><div>{{ botName(item.botId) }}</div><div class="entity-subtitle">{{ workspaceName(item.workspaceId) }}</div></td><td><span class="mono">{{ matcherSummary(item) }}</span></td><td><span class="badge gray">{{ item.priority }}</span><div class="entity-subtitle mono">{{ item.model || '继承 Bot / 平台' }}<template v-if="item.reasoningEffort"> · {{ item.reasoningEffort }}</template></div></td><td><div class="actions"><button class="ghost-button" @click="edit(item)"><Pencil :size="15" />编辑</button><button class="danger-button" aria-label="删除场景" @click="remove(item)"><Trash2 :size="15" /></button></div></td></tr></tbody></table></div><PaginationBar :total="total" :offset="offset" :page-size="pageSize" :loading="loading" @change="move" /></section>
    <div v-if="open" class="dialog-backdrop" @mousedown.self="open = false"><form class="dialog large" @submit.prevent="save"><header class="dialog-header"><div><h2>{{ form.id ? '编辑场景' : '创建场景' }}</h2><p>多个场景命中时优先选择数值更小的场景。</p></div><button type="button" class="icon-button" aria-label="关闭" @click="open = false"><X :size="19" /></button></header><div class="dialog-body">
      ...
      <ModelConfigFields :catalog="catalog" :model="form.model" :reasoning-effort="form.reasoningEffort" inherit-label="继承 Bot / 平台默认模型" :inherited-model="inheritedModel" @update:model="form.model = $event" @update:reasoning-effort="form.reasoningEffort = $event" />
      ...
      <div class="field"><label for="retrieval-boosts">能力检索增强</label><textarea id="retrieval-boosts" v-model="form.retrievalBoostsText" rows="4" class="mono" placeholder="EventBus=20&#10;Argos=15&#10;数据库=10" /><small>每行填写“关键词=权重”。关键词会参与 Skill 和知识召回；权重只对名称、描述、路径或标签中包含该词的 Skill 加分。留空时完全按消息和 Prompt 检索。</small></div>
      <div class="field-grid"><div class="field"><label for="skill-limit">候选 Skill 上限</label><input id="skill-limit" v-model.number="form.skillCandidateLimit" type="number" min="1" max="30" /></div><div class="field"><label for="knowledge-limit">知识资源上限</label><input id="knowledge-limit" v-model.number="form.knowledgeCandidateLimit" type="number" min="1" max="30" /></div></div>
      <div class="field"><label for="minimum-score">最低相关度</label><input id="minimum-score" v-model.number="form.minimumScore" type="number" min="0" max="100" step="1" /><small>建议保持 1 或更高。未达到门槛的 Skill 和知识不会进入 Codex 上下文，也不会出现在消息记录的候选列表中。</small></div>
