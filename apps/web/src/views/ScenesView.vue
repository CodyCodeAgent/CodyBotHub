...
import type { Bot, Scene, SkillPackage, Workspace } from '../types'
...
const form = reactive({ id: '', botId: '', workspaceId: '', name: '', prompt: '', priority: 100, replyMode: 'inherit' as Scene['replyMode'], enabled: true, chatIdsText: '', messageTypes: [] as string[], textIncludesText: '', cardTitleIncludesText: '', skillPackageIds: [] as string[] })
...
onMounted(load)
watch(() => form.botId, () => { if (open.value && !availableWorkspaces.value.some(item => item.id === form.workspaceId)) form.workspaceId = selectedBot.value?.defaultWorkspaceId ?? availableWorkspaces.value[0]?.id ?? '' })
...
<template>
  <div class="page">
    ...
    <div v-if="open" class="dialog-backdrop" @mousedown.self="open = false"><form class="dialog large" @submit.prevent="save"><header class="dialog-header"><div><h2>{{ form.id ? '编辑场景' : '创建场景' }}</h2><p>多个场景命中时优先选择数值更小的场景。</p></div><button type="button" class="icon-button" aria-label="关闭" @click="open = false"><X :size="19" /></button></header><div class="dialog-body">
      ...
      <div class="field-grid"><div class="field"><label for="scene-reply">回复方式</label><select id="scene-reply" v-model="form.replyMode"><option value="inherit">继承 Bot</option><option value="reply">直接回复消息</option><option value="topic">在话题中回复</option></select></div><div class="field"><span class="field-label">状态</span><label class="check-card"><input v-model="form.enabled" type="checkbox" /><span>启用这个场景</span></label></div></div>
      <div class="field"><label for="scene-chat-ids">群 Chat ID</label><textarea id="scene-chat-ids" v-model="form.chatIdsText" rows="3" class="mono" placeholder="oc_xxxxxxxxx\n每行一个，留空表示不限制群" /></div>
