...
const form = reactive({ id: '', botId: '', workspaceId: '', name: '', prompt: '', priority: 100, enabled: true, model: '', reasoningEffort: '', chatIdsText: '', messageTypes: [] as string[], textIncludesText: '', cardTitleIncludesText: '', skillPackageIds: [] as string[] })
...
const edit = (item?: Scene) => {
  Object.assign(form, item ? { ...item, chatIdsText: item.matcher.chatIds.join('\n'), messageTypes: [...item.matcher.messageTypes], textIncludesText: item.matcher.textIncludes.join('\n'), cardTitleIncludesText: item.matcher.cardTitleIncludes.join('\n'), skillPackageIds: [...item.skillPackageIds] } : { id: '', botId: bots.value[0]?.id ?? '', workspaceId: bots.value[0]?.defaultWorkspaceId ?? '', name: '', prompt: '', priority: 100, enabled: true, model: '', reasoningEffort: '', chatIdsText: '', messageTypes: [], textIncludesText: '', cardTitleIncludesText: '', skillPackageIds: [] })
