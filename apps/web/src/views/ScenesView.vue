...
const load = async () => { [items.value, bots.value, workspaces.value, packages.value, messageTypeOptions.value, catalog.value, platform.value] = await Promise.all([api.scenes(), api.bots(), api.workspaces(), api.skillPackages(), api.feishuMessageTypes(), api.models(), api.settings()]) }
onMounted(load)
