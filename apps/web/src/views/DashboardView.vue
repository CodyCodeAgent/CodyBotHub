...
import { api } from '../api'
import type { DashboardOverview, ThreadRoutingRule } from '../types'
...
const load = async () => {
  ...
  try { [stats.value, rules.value] = await Promise.all([api.dashboard(), api.threadRoutingRules()]) }
...
<template>
  <div class="page dashboard-page">
    ...
    <section class="health-strip" :class="{ attention: needsAttention }">
      <div class="health-summary"><span class="health-pulse" /><div><strong>{{ needsAttention ? '有运行项需要留意' : '系统运行正常' }}</strong><p v-if="needsAttention">今日 {{ stats.today.failed }} 条失败，{{ activeJobs }} 条正在排队或执行。</p><p v-else>消息处理与 Thread 队列当前没有待处理异常。</p></div></div>
      <div class="health-stat"><span>当前任务</span><strong>{{ activeJobs }}</strong><small>排队 {{ stats.threads.queuedJobs }} · 执行 {{ stats.threads.processingJobs }}</small></div>
      <div class="health-stat"><span>今日处理</span><strong>{{ stats.today.received }}</strong><small>完成 {{ stats.today.completed }} · 失败 {{ stats.today.failed }}</small></div>
      <div class="health-stat"><span>经验规则</span><strong>{{ enabledRules }}/{{ rules.length }}</strong><small>{{ stats.threads.profiles }} 条可用经验</small></div>
    </section>
    ...
    <section class="analytics-top">
      <article class="panel trend-panel">
        ...
        <div class="trend-chart" role="img" aria-label="近 14 天消息处理趋势图">
