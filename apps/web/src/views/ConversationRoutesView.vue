...
<template>
  <div class="page">
    ...
    <div v-if="selected" class="dialog-backdrop" @mousedown.self="selected = null"><section class="dialog large"><header class="dialog-header"><div><h2>会话线程详情</h2><p>{{ selected.botName }} · {{ routeType(selected) }}</p></div><button class="icon-button" aria-label="关闭" @click="selected = null"><X :size="19" /></button></header><div class="dialog-body">
      <div class="detail-grid"><div><span>Bot</span><strong>{{ selected.botName }}</strong></div><div><span>场景</span><strong>{{ selected.sceneName || '默认路由' }}</strong></div><div><span>工作区</span><strong>{{ selected.workspaceName }}</strong></div><div><span>群</span><strong>{{ selected.chatName || selected.chatId }}</strong></div><div><span>最后活跃</span><strong>{{ formatTime(selected.updatedAt) }}</strong></div><div><span>路由类型</span><strong>{{ routeType(selected) }}</strong></div></div>
      ...
      <div class="record-meta mono">bot: {{ selected.botId }}<br />scene: {{ selected.sceneId || '—' }}<br />workspace: {{ selected.workspaceId }}<br />chat: {{ selected.chatId }}<br />chat mode: {{ selected.chatMode || '—' }}<br />topic: {{ selected.topicId || '—' }}<br />created: {{ formatTime(selected.createdAt) }}</div>
    </div></section></div>
  </div>
</template>
