...
<template>
  <div class="page">
    ...
    <form class="filter-bar" @submit.prevent="search">
      ...
      <div class="filter-search"><Search :size="15" /><input v-model.trim="filters.query" placeholder="搜索记录 ID、消息 ID、Thread ID、内容或回复…" /></div>
    ...
    <div v-if="selected" class="dialog-backdrop" @mousedown.self="selected = null"><section class="dialog large"><header class="dialog-header"><div><h2>消息处理详情</h2><p class="mono">log: {{ selected.id }}</p><p>message: {{ selected.messageId }} · {{ formatTime(selected.receivedAt) }}</p></div><button class="icon-button" aria-label="关闭" @click="selected = null"><X :size="19" /></button></header><div class="dialog-body">
      ...
      <div class="record-meta mono">log: {{ selected.id }}<br />thread: {{ selected.coreThreadId || '—' }}<br />event: {{ selected.eventId }}<br />message: {{ selected.messageId }}<br />chat: {{ selected.chatId }}<br />topic: {{ selected.topicId || '—' }}<br />sender: {{ selected.senderId || '—' }}<br />code root: {{ selected.investigation.codeRoot || '—' }}</div>
    </div></section></div>
  </div>
</template>
