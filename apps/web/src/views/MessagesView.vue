...
<template>
  <div class="page">
    ...
    <form class="filter-bar" @submit.prevent="search">
      ...
      <div class="filter-search"><Search :size="15" /><input v-model.trim="filters.query" placeholder="搜索记录 ID、消息 ID、内容或回复…" /></div>
    ...
    <section class="panel"><div class="panel-header"><h2>处理记录</h2><span class="badge gray">{{ total }} 条</span></div>
      ...
      <div v-else class="table-wrap"><table class="message-table"><thead><tr><th>时间</th><th>消息</th><th>Bot / 路由</th><th>状态</th><th>耗时</th></tr></thead><tbody><tr v-for="item in items" :key="item.id" tabindex="0" @click="selected = item" @keydown.enter="selected = item"><td class="mono">{{ formatTime(item.receivedAt) }}</td><td><div class="entity-title">{{ excerpt(item.inboundContent) }}</div><div class="entity-subtitle">{{ item.messageType }} · message: {{ item.messageId }}</div><div class="entity-subtitle mono">log: {{ item.id }}</div></td><td><div>{{ item.botName }}</div><div class="entity-subtitle">{{ item.sceneName || '默认路由' }} · {{ item.workspaceName }}</div></td><td><span class="badge" :class="item.status === 'completed' ? 'green' : item.status === 'failed' ? 'red' : 'gray'">{{ statusLabel(item.status) }}</span></td><td class="mono">{{ formatDuration(item.durationMs) }}</td></tr></tbody></table></div>
