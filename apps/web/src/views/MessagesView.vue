...
<template>
  <div class="page">
    ...
    <section class="panel"><div class="panel-header"><h2>处理记录</h2><span class="badge gray">{{ total }} 条</span></div>
      ...
      <div v-else class="table-wrap"><table class="message-table"><thead><tr><th>时间</th><th>消息 / Agent Thread</th><th>Bot / 路由</th><th>状态</th><th>耗时</th></tr></thead><tbody><tr v-for="item in items" :key="item.id" tabindex="0" @click="openDetail(item)" @keydown.enter="openDetail(item)"><td class="mono">{{ formatTime(item.receivedAt) }}</td><td><div class="entity-title">{{ excerpt(item.inboundContent) }}</div><div class="entity-subtitle">{{ item.messageType }} · message: {{ item.messageId }}</div><div class="entity-subtitle mono">log: {{ item.id }}</div><div class="entity-subtitle mono">channel: {{ item.threadChannelId || '—' }}</div><div class="entity-subtitle mono">{{ item.runtimeKind === 'traex' ? 'traex' : 'codex' }} thread: {{ item.coreThreadId || '—' }}</div></td><td><div>{{ item.botName }} <span class="badge" :class="item.runtimeKind === 'traex' ? 'green' : 'gray'">{{ item.runtimeKind === 'traex' ? 'TraeX' : 'Codex' }}</span></div><div class="entity-subtitle">{{ item.sceneName || '默认路由' }} · {{ item.workspaceName }}</div><div v-if="item.model" class="entity-subtitle mono">{{ item.model }}<template v-if="item.reasoningEffort"> · {{ item.reasoningEffort }}</template></div></td><td><div class="status-cell"><span class="badge" :class="item.status === 'completed' ? 'green' : item.status === 'failed' ? 'red' : 'gray'">{{ statusLabel(item.status) }}</span><button v-if="item.status === 'failed'" class="ghost-button compact" title="重试这条消息" @click.stop="beginRetry(item)"><RotateCcw :size="13" />重试</button></div></td><td class="mono">{{ formatDuration(item.durationMs) }}</td></tr></tbody></table></div>
