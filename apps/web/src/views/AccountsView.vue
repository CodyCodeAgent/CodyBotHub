...
const total = ref(0), offset = ref(0), loading = ref(false)
...
const load = async () => {
  loading.value = true
  const [accounts, status] = await Promise.all([api.accountsPage(pageSize, offset.value), api.authStatus()])
...
<template>
  <div class="page">
    ...
    <section class="panel"><div class="panel-header"><h2>平台账号</h2><span class="badge gray">{{ total }} 个</span></div>
      <div v-if="!loading && !items.length" class="empty"><UserRound :size="32" /><strong>还没有平台账号</strong></div>
