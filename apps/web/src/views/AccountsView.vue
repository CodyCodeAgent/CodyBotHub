...
<template>
  <div class="page">
    ...
    <section class="panel"><div class="panel-header"><h2>平台账号</h2><span class="badge gray">{{ items.length }} 个</span></div>
      ...
      <div v-else class="table-wrap"><table><thead><tr><th>账号</th><th>用户名</th><th>最近登录</th><th>创建时间</th><th aria-label="操作" /></tr></thead><tbody><tr v-for="item in items" :key="item.id"><td><div class="entity-title mono">{{ item.loginName }}</div><div class="entity-subtitle mono">{{ item.id }}</div></td><td><div>{{ item.displayName }}</div><span v-if="item.id === currentAccountId" class="badge green" style="margin-top:6px">当前账号</span></td><td class="mono">{{ formatTime(item.lastLoginAt) }}</td><td class="mono">{{ formatTime(item.createdAt) }}</td><td><div class="actions"><button class="ghost-button" @click="edit(item)"><Pencil :size="15" />编辑</button><button class="danger-button" :disabled="item.id === currentAccountId" :title="item.id === currentAccountId ? '不能删除当前登录账号' : ''" @click="remove(item)"><Trash2 :size="15" />删除</button></div></td></tr></tbody></table></div>
    </section>
