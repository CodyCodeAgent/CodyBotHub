...
const pageSize = 20
const items = ref<Workspace[]>([]), total = ref(0), offset = ref(0), loading = ref(false), open = ref(false), directoryPickerOpen = ref(false), saving = ref(false), error = ref('')
const form = reactive({ id: '', name: '', path: '', prompt: '' })
const load = async () => { loading.value = true; try { const result = await api.workspacesPage(pageSize, offset.value); items.value = result.items; total.value = result.total } finally { loading.value = false } }
