...
<template>
  <div class="page">
    ...
    <div class="skill-summary">
      <div><strong>{{ total }}</strong><span>当前结果</span></div><div><strong>{{ statusCounts.installed || 0 }}</strong><span>已安装</span></div><div><strong>{{ statusCounts.update_available || 0 }}</strong><span>有更新</span></div><div><strong>{{ statusCounts.local || 0 }}</strong><span>本地 Skill</span></div>
    </div>
    ...
    <section v-if="resources" class="panel">
      <div class="panel-header"><div><h2>工作区资源</h2><p class="panel-description">每轮消息都会重新识别这些目录；新增 Skill 或知识文件后无需重启服务。</p></div><span class="badge gray">{{ resources.knowledgeTotal }} 条知识</span></div>
      ...
      <div v-if="!resources.knowledge.length" class="empty"><BookOpen :size="30" /><strong>没有识别到知识文件</strong><span>支持 knowledge、docs、.codex/knowledge，以及 Skill references 目录。</span></div>
