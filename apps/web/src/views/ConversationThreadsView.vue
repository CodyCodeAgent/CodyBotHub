...
import { GitBranch, RefreshCw, Search, X } from 'lucide-vue-next'
...
<template>
  <div class="page">
    ...
    <section class="panel"><div class="panel-header"><h2>线程映射</h2><span class="badge gray">{{ total }} 条</span></div>
      ...
      <div v-else class="table-wrap"><table class="message-table"><thead><tr><th>最后活跃</th><th>Bot / 引擎 / 会话模式</th><th>群 / 话题</th><th>Thread Channel / Agent Thread</th></tr></thead><tbody><tr v-for="item in items" :key="item.id" tabindex="0" @click="selected = item" @keydown.enter="selected = item"><td class="mono">{{ formatTime(item.updatedAt) }}</td><td><div class="entity-title">{{ item.botName }}</div><div class="entity-subtitle"><span class="badge" :class="item.runtimeKind === 'traex' ? 'green' : 'gray'">{{ item.runtimeKind === 'traex' ? 'TraeX' : 'Codex' }}</span> <span class="badge gray">{{ modeLabel(item) }}</span></div></td><td><div class="entity-title">{{ item.chatName || item.chatId }}</div><div v-if="item.chatName" class="entity-subtitle mono">{{ item.chatId }}</div><div class="entity-subtitle mono">topic: {{ item.topicId || '—' }}</div></td><td><code class="thread-id">channel: {{ item.threadChannelId }}</code><div class="entity-subtitle mono">thread: {{ item.coreThreadId || '尚未创建' }}</div></td></tr></tbody></table></div>
    ...
    <div v-if="selected" class="dialog-backdrop" @mousedown.self="selected = null"><section class="dialog large"><header class="dialog-header"><div><h2>会话线程详情</h2><p>{{ selected.botName }} · {{ modeLabel(selected) }}</p></div><button class="icon-button" aria-label="关闭" @click="selected = null"><X :size="19" /></button></header><div class="dialog-body">
      <div class="detail-grid"><div><span>Bot</span><strong>{{ selected.botName }}</strong></div><div><span>运行引擎</span><strong>{{ selected.runtimeKind === 'traex' ? 'TraeX' : 'Codex' }}</strong></div><div><span>会话模式</span><strong>{{ modeLabel(selected) }}</strong></div><div><span>群</span><strong>{{ selected.chatName || selected.chatId }}</strong></div><div><span>话题</span><strong>{{ selected.topicId || '—' }}</strong></div><div><span>最后活跃</span><strong>{{ formatTime(selected.updatedAt) }}</strong></div><div><span>群类型</span><strong>{{ selected.chatMode || '—' }}</strong></div></div>
      <div class="record-content"><h3>Thread Channel ID</h3><pre>{{ selected.threadChannelId }}</pre></div><div class="record-content"><h3>{{ selected.runtimeKind === 'traex' ? 'TraeX' : 'Codex' }} Thread ID</h3><pre>{{ selected.coreThreadId || '尚未创建' }}</pre></div>
