...
<template>
  <div class="page">
    <header class="page-header"><div><p class="eyebrow">Conversation threads</p><h1>会话线程</h1><p class="page-description">Thread 只由 Bot 的会话模式和飞书会话边界决定。场景、工作区和技能包按每条消息动态路由。</p></div><button class="ghost-button" :disabled="loading" @click="load"><RefreshCw :size="16" />刷新</button></header>
    <section class="route-rules" aria-label="线程映射规则">
      <div class="route-rule"><strong>按群模式</strong><code>Bot + 群 → Thread</code></div>
      <div class="route-rule"><strong>按话题模式</strong><code>Bot + 群 + 话题 → Thread</code></div>
      <div class="route-rule"><strong>每轮路由</strong><code>消息 → 场景 + 工作区 + 技能包</code></div>
    </section>
