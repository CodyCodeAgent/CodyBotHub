...
const form = reactive({ id: '', createMode: 'existing' as 'existing'|'auto', name: '', description: '', appId: '', appSecret: '', prompt: '', operatorIdsText: '', conversationMode: 'chat' as 'chat'|'topic', defaultWorkspaceId: '', workspaceIds: [] as string[] })
...
const edit = (item?: Bot) => {
  Object.assign(form, item ? { ...item, createMode: 'existing', appSecret: '', operatorIdsText: item.operatorIds.join('\n'), workspaceIds: [...item.workspaceIds] } : { id: '', createMode: 'existing', name: '', description: '', appId: '', appSecret: '', prompt: '', operatorIdsText: '', conversationMode: 'chat', defaultWorkspaceId: workspaces.value[0]?.id ?? '', workspaceIds: workspaces.value[0] ? [workspaces.value[0].id] : [] })
