...
const edit = (item?: Bot) => {
  Object.assign(form, item ? { ...item, createMode: 'existing', appSecret: '', operatorIdsText: item.operatorIds.join('\n'), workspaceIds: [...item.workspaceIds] } : { id: '', createMode: 'existing', name: '', description: '', appId: '', appSecret: '', prompt: '', operatorIdsText: '', conversationMode: 'chat', model: '', reasoningEffort: '', defaultWorkspaceId: workspaces.value[0]?.id ?? '', workspaceIds: workspaces.value[0] ? [workspaces.value[0].id] : [] })
...
<template>
  <div class="page">
    ...
    <section class="panel"><div class="panel-header"><h2>全部 Bot</h2><span class="badge gray">{{ total }} 个</span></div><div v-if="!loading && !items.length" class="empty"><BotIcon :size="32" /><strong>还没有 Bot</strong><span>关联一个飞书应用，或之后通过自动注册流程创建。</span></div><div v-else class="table-wrap"><table><thead><tr><th>Bot</th><th>飞书应用</th><th>默认工作区</th><th>会话模式 / 模型</th><th aria-label="操作" /></tr></thead><tbody><tr v-for="item in items" :key="item.id"><td><div class="entity-title">{{ item.name }}</div><div class="entity-subtitle">{{ item.description || '暂无描述' }}</div></td><td><div class="mono">{{ item.appId || '待配置' }}</div><div class="entity-subtitle"><KeyRound :size="11" style="vertical-align:-2px" /> {{ item.hasAppSecret ? '凭据已加密' : '未配置 Secret' }}</div></td><td><span class="badge">{{ workspaceName(item.defaultWorkspaceId) }}</span><div class="entity-subtitle">可访问 {{ item.workspaceIds.length }} 个工作区</div></td><td><span class="badge gray">{{ item.conversationMode === 'topic' ? '一个话题一个 Thread' : '一个群一个 Thread' }}</span><div class="entity-subtitle mono">{{ item.model || '继承平台模型' }}<template v-if="item.reasoningEffort"> · {{ item.reasoningEffort }}</template></div></td><td><div class="actions"><button class="ghost-button" @click="edit(item)"><Pencil :size="15" />编辑</button><button class="danger-button" aria-label="删除 Bot" @click="remove(item)"><Trash2 :size="15" /></button></div></td></tr></tbody></table></div><PaginationBar :total="total" :offset="offset" :page-size="pageSize" :loading="loading" @change="move" /></section>
    <div v-if="open" class="dialog-backdrop" @mousedown.self="open = false"><form class="dialog large" @submit.prevent="save"><header class="dialog-header"><div><h2>{{ form.id ? '编辑 Bot' : '创建 Bot' }}</h2><p>可以接入已有应用，也可以扫码自动创建飞书应用。</p></div><button type="button" class="icon-button" aria-label="关闭" @click="open = false"><X :size="19" /></button></header><div class="dialog-body">
      ...
      <ModelConfigFields :catalog="catalog" :model="form.model" :reasoning-effort="form.reasoningEffort" inherit-label="继承平台默认模型" :inherited-model="platform.defaultModel || catalog.defaultModel" @update:model="form.model = $event" @update:reasoning-effort="form.reasoningEffort = $event" />
