...
import type { AgentRuntimeKind, Bot, ModelCatalog, PlatformSettings, ProvisioningJob, Workspace } from '../types'
const pageSize = 20
const items = ref<Bot[]>([]), workspaces = ref<Workspace[]>([]), total = ref(0), offset = ref(0), loading = ref(false), open = ref(false), saving = ref(false), error = ref('')
...
const form = reactive({ id: '', createMode: 'existing' as 'existing'|'auto', name: '', description: '', appId: '', appSecret: '', prompt: '', operatorIdsText: '', conversationMode: 'chat' as 'chat'|'topic', runtimeKind: 'codex' as AgentRuntimeKind, model: '', reasoningEffort: '', defaultWorkspaceId: '', workspaceIds: [] as string[] })
const canCreate = computed(() => workspaces.value.length > 0)
const load = async () => { loading.value = true; try { const [page, workspaceItems, models, settings] = await Promise.all([api.botsPage(pageSize, offset.value), api.workspaces(), api.models(), api.settings()]); items.value = page.items; total.value = page.total; workspaces.value = workspaceItems; catalog.value = models; platform.value = settings } finally { loading.value = false } }
...
const loadCatalog = async (kind: AgentRuntimeKind) => {
  try { catalog.value = await api.models(kind) }
  catch (e) { catalog.value = { items: [], defaultModel: '', defaultReasoningEffort: '' }; if (open.value) error.value = `${kind === 'traex' ? 'TraeX' : 'Codex'} 运行时暂不可用：${e instanceof Error ? e.message : '无法读取模型'}` }
}
const edit = (item?: Bot) => {
  Object.assign(form, item ? { ...item, createMode: 'existing', appSecret: '', operatorIdsText: item.operatorIds.join('\n'), workspaceIds: [...item.workspaceIds] } : { id: '', createMode: 'existing', name: '', description: '', appId: '', appSecret: '', prompt: '', operatorIdsText: '', conversationMode: 'chat', runtimeKind: 'codex', model: '', reasoningEffort: '', defaultWorkspaceId: workspaces.value[0]?.id ?? '', workspaceIds: workspaces.value[0] ? [workspaces.value[0].id] : [] })
  error.value = ''; open.value = true; void loadCatalog(form.runtimeKind)
}
const changeRuntime = async (kind: AgentRuntimeKind) => { form.runtimeKind = kind; form.model = ''; form.reasoningEffort = ''; error.value = ''; await loadCatalog(kind) }
...
<template>
  <div class="page">
    <header class="page-header"><div><p class="eyebrow">Channel accounts</p><h1>飞书 Bot</h1><p class="page-description">接入已有飞书应用并为它配置默认工作区、可访问的其他工作区、基础 Prompt 和会话模式。</p></div><button class="button" :disabled="!canCreate" :title="canCreate ? '' : '请先创建工作区'" @click="edit()"><Plus :size="17" />创建 Bot</button></header>
    ...
    <section class="panel"><div class="panel-header"><h2>全部 Bot</h2><span class="badge gray">{{ total }} 个</span></div><div v-if="!loading && !items.length" class="empty"><BotIcon :size="32" /><strong>还没有 Bot</strong><span>关联一个飞书应用，或之后通过自动注册流程创建。</span></div><div v-else class="table-wrap"><table><thead><tr><th>Bot</th><th>飞书应用</th><th>默认工作区</th><th>运行引擎 / 会话</th><th aria-label="操作" /></tr></thead><tbody><tr v-for="item in items" :key="item.id"><td><div class="entity-title">{{ item.name }}</div><div class="entity-subtitle">{{ item.description || '暂无描述' }}</div></td><td><div class="mono">{{ item.appId || '待配置' }}</div><div class="entity-subtitle"><KeyRound :size="11" style="vertical-align:-2px" /> {{ item.hasAppSecret ? '凭据已加密' : '未配置 Secret' }}</div></td><td><span class="badge">{{ workspaceName(item.defaultWorkspaceId) }}</span><div class="entity-subtitle">可访问 {{ item.workspaceIds.length }} 个工作区</div></td><td><span class="badge" :class="item.runtimeKind === 'traex' ? 'green' : 'gray'">{{ item.runtimeKind === 'traex' ? 'TraeX' : 'Codex' }}</span><span class="badge gray" style="margin-left:6px">{{ item.conversationMode === 'topic' ? '一个话题一个 Thread' : '一个群一个 Thread' }}</span><div class="entity-subtitle mono">{{ item.model || (item.runtimeKind === 'traex' ? 'TraeX 默认模型' : '继承平台模型') }}<template v-if="item.reasoningEffort"> · {{ item.reasoningEffort }}</template></div></td><td><div class="actions"><button class="ghost-button" @click="edit(item)"><Pencil :size="15" />编辑</button><button class="danger-button" aria-label="删除 Bot" @click="remove(item)"><Trash2 :size="15" /></button></div></td></tr></tbody></table></div><PaginationBar :total="total" :offset="offset" :page-size="pageSize" :loading="loading" @change="move" /></section>
    <div v-if="open" class="dialog-backdrop" @mousedown.self="open = false"><form class="dialog large" @submit.prevent="save"><header class="dialog-header"><div><h2>{{ form.id ? '编辑 Bot' : '创建 Bot' }}</h2><p>可以接入已有应用，也可以扫码自动创建飞书应用。</p></div><button type="button" class="icon-button" aria-label="关闭" @click="open = false"><X :size="19" /></button></header><div class="dialog-body">
      <div class="field-grid"><div class="field"><label for="bot-name">名称</label><input id="bot-name" v-model.trim="form.name" required placeholder="例如：研发助手" /></div><div class="field"><label for="bot-conversation-mode">会话模式</label><select id="bot-conversation-mode" v-model="form.conversationMode"><option value="chat">一个群一个 Thread</option><option value="topic">一个话题一个 Thread</option></select></div></div>
      <div class="field"><span class="field-label">Agent 运行引擎</span><div class="check-grid"><label class="check-card"><input :checked="form.runtimeKind === 'codex'" type="radio" value="codex" @change="changeRuntime('codex')" /><span><strong>Codex</strong><small>使用 Codex App Server 与 Codex Thread</small></span></label><label class="check-card"><input :checked="form.runtimeKind === 'traex'" type="radio" value="traex" @change="changeRuntime('traex')" /><span><strong>TraeX</strong><small>使用 TraeX App Server 与独立 Thread</small></span></label></div><small>修改引擎后，后续群或话题会创建对应引擎的 Thread；已有历史记录会保留。</small></div>
      ...
      <ModelConfigFields :catalog="catalog" :model="form.model" :reasoning-effort="form.reasoningEffort" :inherit-label="form.runtimeKind === 'traex' ? '使用 TraeX 默认模型' : '继承平台默认模型'" :inherited-model="form.runtimeKind === 'traex' ? catalog.defaultModel : platform.defaultModel || catalog.defaultModel" @update:model="form.model = $event" @update:reasoning-effort="form.reasoningEffort = $event" />
