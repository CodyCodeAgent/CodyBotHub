...
export interface Bot { id: string; name: string; description: string; appId: string; hasAppSecret: boolean; prompt: string; permissions: string[]; operatorIds: string[]; conversationMode: 'chat' | 'topic'; botMessagePolicy: BotMessagePolicy; mentionSourceBot: boolean; botSourceAllowlist: string[]; maxBotReplyDepth: number; runtimeKind: AgentRuntimeKind; model: string; reasoningEffort: string; defaultWorkspaceId: string; workspaceIds: string[]; createdAt: string; updatedAt: string }
...
export interface SkillPackage { id: string; workspaceId: string; name: string; description: string; prompt: string; skills: string[]; toolPackageIds: string[]; fallbackMode: 'package_first' | 'mixed' | 'package_only'; createdAt: string; updatedAt: string }
export interface BitsRpcConfig { service: string; method: string; vregion: string; environment: string; cluster: string; requestTemplate: Record<string, unknown> }
export interface Tool { id: string; workspaceId: string; name: string; description: string; executorType: 'bits_rpc'|'command'; command: string; argumentsTemplate: string[]; inputSchema: Record<string, unknown>; rpcConfig: BitsRpcConfig; scriptLanguage: 'python'|'shell'|'node'|''; scriptContent: string; scriptVersion: number; timeoutSeconds: number; enabled: boolean; createdAt: string; updatedAt: string }
export interface ToolScriptVersion { toolId: string; version: number; language: 'python'|'shell'|'node'; content: string; createdAt: string }
export interface ToolPackageStep { toolId: string; toolName: string; phase: 'precheck'|'execute'|'verify'; position: number; arguments: Record<string, unknown> }
export interface ToolPackage { id: string; workspaceId: string; name: string; description: string; prompt: string; approvalRequired: boolean; approverIds: string[]; cardTitle: string; cardDescription: string; enabled: boolean; steps: ToolPackageStep[]; createdAt: string; updatedAt: string }
export interface ToolPackageExecution { id: string; callId: string; toolPackageId: string; toolPackageName: string; botId: string; sceneId: string; chatId: string; topicId: string; threadChannelId: string; coreThreadId: string; sourceMessageId: string; status: 'awaiting_approval'|'queued'|'running'|'completed'|'failed'|'rejected'; arguments: Record<string, unknown>; reason: string; requestedBy: string; approvedBy: string; cardMessageId: string; result: unknown; error: string; createdAt: string; updatedAt: string; completedAt: string }
...
export interface ThreadProfile { coreThreadId: string; threadChannelId: string; botId: string; workspaceId: string; sceneId: string; runtimeKind: AgentRuntimeKind; title: string; fields: Record<string, string>; normalizedText: string; experienceSummary: string; messageCount: number; lastMessageLogId: string; lastActiveAt: string; updatedAt: string }
...
export interface ThreadChannel { id: string; botId: string; botName: string; runtimeKind: AgentRuntimeKind; coreThreadId: string; bindingCount: number; queuedJobs: number; processingJobs: number; createdAt: string; updatedAt: string }
